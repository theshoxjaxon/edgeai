    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="relative w-full max-w-md bg-gradient-to-br from-[#011627] to-[#011627] border border-secondary/20 rounded-2xl p-8 shadow-2xl"
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === 'signup' && (
              <div>
                <label className="block text-secondary text-sm mb-2">{t('auth.fullName')}</label>
            ...
            <div>
              <label className="block text-secondary text-sm mb-2">{t('auth.email')}</label>
              <div className="relative">
            ...
            <div>
              <label className="block text-secondary text-sm mb-2">{t('auth.password')}</label>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full pl-12 pr-12 py-3 bg-base-200 border border-secondary/20 rounded-lg text-base-content placeholder-secondary/50 focus:outline-none focus:border-primary/50"
                  required
                />
                <button
                  type="button"
            ...
            {mode === 'login' && (
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" className="w-4 h-4 rounded border-secondary/20 bg-base-200 text-primary focus:ring-[#CCFF00]" />
                  <span className="text-secondary text-sm">{t('auth.rememberMe')}</span>
                </label>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-4 bg-primary text-primary-content font-bold rounded-lg hover:brightness-90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {isLoading ? (
          ...
          <div className="flex items-center gap-4 my-6">
            ...
            <span className="text-secondary text-sm">{t('auth.or')}</span>
            <div className="flex-1 h-px bg-secondary/20" />
          </div>

          {/* Social Login */}
          <div className="grid grid-cols-2 gap-3">
            <button className="flex items-center justify-center gap-2 py-3 bg-base-200 border border-secondary/20 rounded-lg text-base-content hover:border-primary/40 transition-colors">
