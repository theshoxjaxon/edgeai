...
export function BetTracking({ onLogout }: BetTrackingProps) {
  ...
  if (isLoading || !dashboardData) {
    return <div className="min-h-screen bg-base-100 pt-20 flex items-center justify-center text-primary">Loading bets...</div>;
  }
    ...
    <div className="min-h-screen bg-base-100 pt-20">
      <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-12 py-8">
        <div className="max-w-7xl mx-auto">
          ...
          >
            <div>
              <h1 className="text-3xl font-bold text-base-content">{t('betTracking.title')}</h1>
              <p className="text-secondary mt-1">{t('betTracking.desc')}</p>
            </div>
            <div className="flex items-center gap-3">
              <button 
                onClick={() => setShowAddModal(true)}
                className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-content font-semibold rounded-lg hover:brightness-90 transition-colors"
              >
                <Plus className="w-4 h-4" />
                {t('betTracking.addBet')}
              </button>
              <button className="flex items-center gap-2 px-4 py-2 bg-base-100 border border-secondary/20 text-secondary rounded-lg hover:border-primary/40 transition-colors">
          ...
          >
            {statsCards.map((stat, index) => (
              <div
                key={index}
                className="bg-gradient-to-br from-[#011627] to-[#0A2A3A] border border-secondary/10 rounded-xl p-6 hover:border-primary/30 transition-all"
              >
          ...
          >
            ...
            <div className="bg-gradient-to-br from-[#011627] to-[#0A2A3A] border border-secondary/10 rounded-xl p-6">
              <h3 className="text-lg font-bold text-base-content mb-6">{t('betTracking.profitOverTime')}</h3>
              <div className="h-64">
            ...
            <div className="bg-gradient-to-br from-[#011627] to-[#0A2A3A] border border-secondary/10 rounded-xl p-6">
              <h3 className="text-lg font-bold text-base-content mb-6">{t('betTracking.winLossDist')}</h3>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
              ...
              <div className="flex justify-center gap-6 mt-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-secondary text-sm">{t('betTracking.wins')} ({mockStats.wins})</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-red-500" />
                  <span className="text-secondary text-sm">{t('betTracking.losses')} ({mockStats.losses})</span>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          >
            {/* Table Header */}
            <div className="p-4 border-b border-secondary/10 flex flex-col sm:flex-row gap-4">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-base-200 border border-secondary/20 rounded-lg text-base-content placeholder-secondary/50 focus:outline-none focus:border-primary/50 text-sm"
                />
              </div>
              <select
                value={filter}
                onChange={(e) => setFilter(e.target.value)}
                className="px-4 py-2 bg-base-200 border border-secondary/20 rounded-lg text-base-content text-sm focus:outline-none focus:border-primary/50"
              >
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-[#00F5FF]/10">
                  {filteredBets.map((bet) => (
                    <tr key={bet.id} className="hover:bg-base-200 transition-colors">
                      <td className="py-4 px-4">
                        <p className="text-base-content font-medium">{bet.match}</p>
                        <p className="text-secondary text-xs">{bet.date}</p>
                      </td>
                      <td className="py-4 px-4">
                        <span className="text-secondary">{bet.selection}</span>
                      </td>
                      <td className="py-4 px-4 text-center">
                        <span className="text-base-content">@{bet.odds}</span>
                      </td>
                      <td className="py-4 px-4 text-center">
                        <span className="text-base-content">${bet.stake}</span>
                      </td>
                      <td className="py-4 px-4 text-center">
            ...
            {filteredBets.length === 0 && (
              <div className="p-12 text-center">
                ...
                <h3 className="text-base-content font-semibold mb-2">{t('betTracking.noBetsFound')}</h3>
                <p className="text-secondary">{t('betTracking.adjustFilters')}</p>
              </div>
            )}
          </motion.div>
        </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <motion.div
            ...
            className="w-full max-w-md bg-gradient-to-br from-[#011627] to-[#011627] border border-secondary/20 rounded-2xl p-6"
          >
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-base-content">{t('betTracking.addNewBet')}</h3>
              <button 
            ...
            <form className="space-y-4">
              <div>
                <label className="block text-secondary text-sm mb-2">Match</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-secondary text-sm mb-2">Selection</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-secondary text-sm mb-2">Odds</label>
                  <input
                    type="number"
                    step="0.01"
                    placeholder="2.00"
                ...
                <div>
                  <label className="block text-secondary text-sm mb-2">Stake ($)</label>
                  <input
                    type="number"
                    placeholder="50"
                    className="w-full px-4 py-3 bg-base-200 border border-secondary/20 rounded-lg text-base-content placeholder-secondary/50 focus:outline-none focus:border-primary/50"
              ...
              <button
                type="button"
                onClick={() => setShowAddModal(false)}
                className="w-full py-3 bg-primary text-primary-content font-bold rounded-lg hover:brightness-90 transition-colors"
              >
                Add Bet
              </button>
            </form>
